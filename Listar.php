...
$sql = "SELECT id, Produtos, Valor FROM loja WHERE 1=1";
$params = [];
$types = '';
...
<title>Listar Produtos – NexusDigital</title>
...
tbody tr {
    border-bottom: 1px solid rgba(90,59,28,0.12);
}
...
<main class="main">
    ...
    <div class="page-header">
        ...
        <a href="Cad.php" class="btn-novo">+ Novo Produto</a>
    </div>
    ...
    <p class="result-info">
        <?php if ($busca || $precoMin !== null || $precoMax !== null): ?>
            Filtro ativo — <strong><?= count($produtos) ?></strong>
            <?= count($produtos) === 1 ? 'produto encontrado' : 'produtos encontrados' ?>
        <?php else: ?>
            Total: <strong><?= count($produtos) ?></strong>
            <?= count($produtos) === 1 ? 'produto cadastrado' : 'produtos cadastrados' ?>
        <?php endif; ?>
    </p>
        ...
        <table>
            <thead>
                <tr>
                    <th>#</th>
                    <th>Produto</th>
                    <th class="center">Preço (R$)</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($produtos as $i => $p): ?>
                <tr>
                    <td style="color:#a07850; font-size:12px;"><?= $i + 1 ?></td>
